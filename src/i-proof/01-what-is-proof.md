...
\\[
\forall n \in \mathbb{N}. p(n)\ is\ prime. \tag{1.2}
\\]
...
## "If and Only If"による証明

多くの数学的定理が2つの命題が論理的に同値であることを主張している；すなわち，もし一方が成り立ちかつその場合に限り（if and only if），他方が成り立つ．以下の例は数千年前から知られている：

> 2つの三角形は，2つの辺とその間に挟まれる角度が等しいとき，またそのときに限りすべての辺が等しくなる．

"if and only if"は"iff"と略されることが多い．

### 1.6.1 方法1：互いの命題がお互いを含意する

\\(P\ IFF\ Q\\)は，2つの文\\(PならばQ\\)と\\(QならばP\\)が成り立つことと同値である．よって"iff"の証明は，2つの含意命題を証明することでできる．

1. "\\(PならばQ\\)およびその逆を証明する"と書く．
2. "初めに，\\(PならばQ\\)を示す．"と書き，証明する．
3. "次に，\\(QならばP\\)を示す．"と書き，証明する．

### 1.6.2 方法2：Iffの連鎖を構成する

この方法で\\(P\\)が真であるとき，またそのときに限り\\(Q\\)が真であると証明するためには：

1. "我々はif-and-only-if含意の連鎖を構成する．"と書く．
2. \\(P\\)が2番目の文と等しいことを証明し，それが3番目の文と等しいことを証明し，\\(Q\\)に到達するまでこれを繰り返す．

これは方法1と比べて独創性を求められる場合がある．しかし最終的には短い，エレガントな証明にできうる．

**例**

値の系列\\(x_1, x_2, \cdots, x_n\\)の**標準偏差**は次のように定義できる：
...
**定理 1.6.1．** 値の系列\\(x_1, x_2, \cdots, x_n\\)のすべての値が平均値と等しいとき，またそのときに限り標準偏差がゼロとなる．

すべての生徒のテストの点が平均値と等しいとき，またそのときに限り得点の標準偏差はゼロになる．

**証明．** 我々は"iff"含意の連鎖を構成する．まず式(1.3)をゼロと等しく置いた文から開始する：

\\[
\sqrt{\frac{(x_1 - \mu)^2 + (x_2 - \mu)^2 + \cdots + (x_n - \\mu)^2}{n}} = 0 \tag{1.4}
\\]

ゼロは平方根がゼロと等しくなる唯一の数であるため，式(1.4)でありそのときに限り，以下が成立する：

\\[
(x_1 - \mu)^2 + (x_2 - \mu)^2 + \cdots + (x_n - \mu)^2  = 0 \tag{1.5}
\\]

実数の自乗は常に非負である．よって式(1.5)の左辺の全ての項は非負である．これは(1.5)が成り立つとき，またそのときに限り，以下が成立することを意味する：

\\[
式(1.5)の左辺の全ての項はゼロである．　\tag{1.6}
\\]

しかし項\\(x_i - \mu)^2\\)は\\(x_i = \mu\\)であるとき，またそのときに限りゼロである．よって(1.6)であるとき，またそのときに限り，
...
複雑な証明を場合分けして，それぞれの場合に応じて証明を行うことは，汎用的で有用な証明戦略である．以下にこの証明が有力な具体例を示す．

ある2人の人が，面識があるかないかという観点から見てみる．あるグループ内ですべての人（任意の2人）が面識がある場合，そのグループは**クラブである**という．グループ内ですべての人間がお互いに面識がないとき，そのグループは**他人である**という．

**定理．** 6人を適当にグループ分けすると，少なくとも3人のクラブまたは少なくとも3人の他人のグループが存在する．

**証明** \\(x\\)を6人のうちの誰か一人を表すものとする．すると\\(x\\)と他の5人との面識について2つの場合を考えることができる．

**場合1.** 少なくとも3人が\\(x\\)に会ったことがある．

**場合2.** 少なくとも3人が\\(x\\)に会ったことがない．

1の場合をさらに分割する．

**場合 1.1.** \\(x\\)以外の人は誰も互いに面識がない．すると少なくとも3人の他人グループが存在することがわかる．よってこの場合は定理を満たす．

**場合 1.2.** \\(x\\)以外の人のうち少なくとも誰か2人が面識がある．するとその2人は\\(x\\)と面識があるので，少なくとも3人のクラブができる．よってこの場合も定理を満たす．

2の場合をさらに分割する．

**場合 2.1** \\(x\\)以外の人が全員お互いに面識がある．すると少なくとも3人のクラブが存在することがわかる．よってこの場合は定理を満たす．

**場合 2.2** \\(x\\)以外の人のうち少なくとも誰か2人が面識がない．するとその2人は\\(x\\)と面識がないので少なくとも3人の他人グループが存在する．よってこの場合も定理を満たす．

## 背理法による証明

**背理法**，あるいは**間接証明**とは，ある命題が偽であるとしたとき，そこから偽の事実が真であると導かれたとする．すると偽の事実は定義上は真にはなりえないので，その命題は真でなければならない，とする論証ができる．

その名前の通り，間接証明は複雑になりがちなので，直接証明ができるならばそちらが推奨される．

**手法：** 命題\\(P\\)を背理法で証明するには，

1. "背理法による証明を行う．"と書く．
2. "\\(P\\)が偽であると仮定する．"と書く．
3. 偽であると知られている推論にたどり着く（矛盾）．
4. "これは矛盾である．ゆえに，\\(P\\)は真である．"と書く．
...
**以下の練習問題はメモであり，問題文を正確に記述していません**

### 講義問題

### 問題 1.1

ピタゴラスの定理を言葉なしで証明するのが目的の回．4つの合同な直角三角形と，意味ありげな正方形が用意されている（原著のFigure 1.1を参照）．\\(c \times c\\)の正方形になるようこれらの図形を配置せよ，という問題．次に\\(a \times a\\)と\\(b \\times b\\)．
...
(b) もし\\(1 = -1\\)なら\\(2 = 1\\)であることを証明せよ．

(c) 正の実数\\(r\\)および\\(s\\)について\\(\sqrt{rs} = \sqrt{r}\sqrt{s}\\)を証明せよ．

### 問題 1.3

偽の主張とその間違った証明が提示される．どこが間違っているか，バグを特定せよ．

### 問題 1.4

算術平均と幾何平均の不等式の証明の間違いを指摘させる問題．
...
と仮定する．これらの整数に関する主張から別の主張を証明する．

### 試験問題

### 問題 1.11

\\(a^{\sqrt{3}}\\)が有理数になるような無理数\\(a\\)が存在することを証明せよ．
...
任意の整数\\(n > 0\\)について，もし\\(a^n\\)が偶数ならば，\\(a\\)も偶数である．

ヒント：背理法

### 問題 1.13

\\(a \cdot b =  n \\)ならば，\\(a,b \leq \sqrt{n}\\)を証明せよ．ここで\\(a,b,n\\)は非負実数である．
...
(a) \\(n^2\\)が偶数のとき，\\(n\\)も偶数である理由を説明せよ．

(b) \\(n^3\\)が3の倍数のとき，\\(n\\)も3の倍数である理由を説明せよ．

### 問題 1.15

2つの正整数\\(m, n\\)があり\\(n^2\\)が\\(m\\)の倍数で，\\(n\\)は\\(m\\)の倍数でないとする．mがn未満であることをどのように説明できるか？

### 問題 1.16

1.8.1の\\(\sqrt{2}\\)が無理数である証明を一般化できるか？例えば\\(\sqrt{3}\\)はどうか．

### 問題 1.17

\\(log_{4}{6}\\)が無理数であることを証明せよ．

### 問題 1.18

\\(\sqrt{3} + \sqrt{2}\\)が無理数であることを証明せよ（背理法）．

ヒント： \\(\sqrt{3} + \sqrt{2})(\sqrt{3} - \sqrt{2})\\)

### 問題 1.19

問題 1.16の一般化として，以下を考える．

**補題．** 以下の多項式の係数は整数であるとする．

\\[
a_0 + a_1 x + a_2 x^3 + \cdots + a_{m-1} x^{m-1} + x^m
\\]

この多項式の実数根は整数または無理数になる．

(a) \\(k\\)がある整数の\\(m\\)乗でないとき，この補題が\\(\sqrt[m]{k}\\)を直ちに含意する理由を示せ．

(b) 上の補題を証明せよ．

### 問題 1.20

\\(log_{9}{12}\\)が無理数であることを証明せよ．

### 問題 1.21

\\(log_{12}{18}\\)が無理数であることを証明せよ．

### 問題 1.22

\\(\sqrt[3]{7^2}\\)が無理数であることの証明に関連する式を6つ提示して，それが証明のどこで現れるか．
...
\\(\sqrt{2}\\)が無理数であることの別証明．証明の概略が示されているが，これは大学講師向けに簡略化されているので詳しく埋めよ，という問題．

### 問題 1.25

素数を生成する多項式は存在しないことの証明．
